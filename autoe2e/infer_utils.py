...
from autoe2e.llm_api_call import haiku_chain, openai_embeddings, sonnet_chain
...
from autoe2e.utils import extract_response_content, geometric_score, logger, png_to_base64
...
def extract_state_context(
    ...
    screenshot_path = f"{crawl_context.settings.temp_dir}/screenshot_{state_id}.png"
    save_screenshot(crawl_context.page, screenshot_path)
        ...
        create_context_user_messages(
            {
                ...
                else prev_state.get_context(),
                "previous_action": "None. This is the first state."
                if prev_action is None
                else prev_action.element.outerHTML,
            },
...
def extract_action_functionalities(
    state: State, action: Action, prev_action: Action | None = None
) -> list[str]:
    res = sonnet_chain(
        FUNCTIONALITY_EXTRACTION_SYSTEM_PROMPT,
        create_functionality_user_messages(
...
def extract_action_functionalities_dict(state, action, prev_action=None) -> list[str]:
    res = sonnet_chain(
        FUNCTIONALITY_EXTRACTION_SYSTEM_PROMPT,
        create_functionality_user_messages(
            state["context"],
...
def map_similar_func_to_exact_match(func_info):
    ...
    if len(exact_match_indices) > 0:
        match = {"match": True, "match_index": exact_match_indices, "combined_text": text}
    else:
        match = {"match": False}
    ...
    if match["match"]:
        if isinstance(match["match_index"], int):
...
def match_update(match):
    ...
    func_db.update_one(
        filter={"app": os.getenv("APP_NAME"), "_id": match["match_id"][0]},
        update={
            "$set": {
    ...
    if len(match["match_id"]) > 1:
        ...
        func_db.delete_many(
            {"app": {"$eq": os.getenv("APP_NAME")}, "_id": {"$in": match["match_id"][1:]}}
        )
        ...
        action_func_db.update_many(
            ...
            update={"$set": {"func_pointer": str(match["match_id"][0])}},
            upsert=False,
        )
...
def insert_functionalities(functionalities: list[str]):
    ...
    matches = map(
        ...
        zip(range(len(functionalities)), functionalities, embeddings, similar_funcs),
    )
...
def update_functionality_score(prev_state, prev_action, curr_state, curr_action):
    ...
    for curr_func in curr_action_funcs:
        corresponding_func_in_prev = list(
            filter(lambda x: x["func_pointer"] == curr_func["func_pointer"], prev_action_funcs)
        )
        prev_score = (
            geometric_score(None)
            if len(corresponding_func_in_prev) == 0
    ...
    for _id, diff in func_score_updates.items():
        func_db.update_one(
            filter={"app": os.getenv("APP_NAME"), "_id": ObjectId(_id), "final": False},
            update={"$inc": {"score": diff}},
            upsert=False,
        )
...
def mark_final_functionalities(curr_state, curr_action):
    ...
    retreived_funcs = list(
        func_db.find(
            {
                "app": {"$eq": os.getenv("APP_NAME")},
                "_id": {"$in": list(map(lambda x: ObjectId(x["func_pointer"]), curr_action_funcs))},
            }
        )
    )
    ...
    for i in range(len(finality)):
        if finality[i]:
            func_db.update_one(
                filter={"app": os.getenv("APP_NAME"), "_id": retreived_funcs[i]["_id"]},
                update={
                    "$set": {
                        "final": True,
                    }
...
def is_action_critical(action: Action) -> bool:
    ...
    res = haiku_chain(CRITICAL_ACTION_SYSTEM_PROMPT, create_simple_user_messages(element_html))
...
def create_form_filling_values(action: Action):
    ...
    res = sonnet_chain(FORM_VALUE_SYSTEM_PROMPT, create_simple_user_messages(element_html))
