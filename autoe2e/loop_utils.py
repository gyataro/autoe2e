...
def get_next_action(crawl_context: CrawlContext):
    ...
    highest_funcs = list(
        func_db.find({"app": os.getenv("APP_NAME"), "final": False, "executable": True})
        .sort({"score": -1})
        .limit(1)
    )
    ...
    if len(connected_actions) == 0:
        func_db.update_one(
            filter={"app": os.getenv("APP_NAME"), "_id": highest_func["_id"]},
            update={"$set": {"executable": False}},
            upsert=False,
        )
...
def flag_action_to_stop_execution(state: State, action: Action, feature_id: str | None = None):
    if feature_id is None:
        action_func_db.update_many(
            ...
            update={"$set": {"should_execute": False}},
            upsert=False,
        )
    else:
        action_func_db.update_many(
...
def extract_state_action_features(crawl_context: CrawlContext, state: State):
    ...
    for action in actions:
        ...
        if len(state.crawl_path) > 0:
            ...
            update_functionality_score(
                state.crawl_path.get_state(-1), state.crawl_path.get_action(-1), state, action
            )
